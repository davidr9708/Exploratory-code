...
graph_factor_factor <- function(data, factor_var, outcome_var, graph = 'point'){
  ...
  prop_outcome <- cumsum(prop.table(table(data[, outcome_var][[1]])))*100
  ...
  for(name in factor_var){
    ...
    if(graph == 'bar'){
      plot <- summary_data %>% 
      ggplot(aes_string(x = 'percent', y = name, fill = outcome_var)) +
    ...
    if(graph == 'point'){ 
     plot <- summary_data %>% 
        ggplot(aes_string(x = 'percent', y = name, color = outcome_var)) +
        geom_point(size = 3) + 
        geom_vline(xintercept = prop_outcome, linetype='dashed', color = 'gray')  +
        scale_x_continuous(expand = c(0, 0), limits = c(0,120), breaks = seq(0,100, 25)) + 
        ggtitle(name) +
        facet_wrap(reformulate(outcome_var)) +
        theme(plot.background  = element_blank(),
              panel.background = element_blank(),
              axis.title = element_blank(),
              axis.ticks.y = element_blank()) +
